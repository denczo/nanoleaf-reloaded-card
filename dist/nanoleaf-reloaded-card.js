...
/**
  ...
  `;static getConfigElement(){return document.createElement("nanoleaf-reloaded-card-editor")}static getStubConfig(t){return function(t){const e={};for(const t of vt)e[t.key]="";const i=t?.entities??{},s=t?.states??{};for(const t of Object.keys(s)){if("nanoleaf_reloaded"!==i[t]?.platform)continue;const s=t.split(".")[0],n=t.toLowerCase();"light"===s?/base|color|colour/.test(n)?e.color_entity=t:e.light_entity||(e.light_entity=t):"sensor"===s?/color|colour/.test(n)?e.panel_colors_entity=t:/layout/.test(n)&&(e.layout_sensor=t):"select"===s?e.pattern_entity||(e.pattern_entity=t):"number"===s&&(/spread/.test(n)?e.spread_entity=t:/bright/.test(n)&&(e.brightness_entity=t))}return e}(t)}setConfig(t){this._devices=function(t){const e=Array.isArray(t.devices),i=pt.some(e=>e in t);if(e&&i)throw new Error('nanoleaf-card: use either "devices" or top-level entity keys, not both');if(e){if(0===t.devices.length)throw new Error('nanoleaf-card: "devices" is empty');return t.devices.forEach((t,e)=>{ut(t,t.name||`devices[${e}]`)}),t.devices}return ut(t),[t]}(t),this._config=t,this._storageKey="nanoleaf-card:"+this._devices.map(t=>t.light_entity).join(","),this._activeIndex=_t(this._loadIndex(),this._devices.length),this._debouncedSetBrightness=dt(t=>this._setValue(this._activeDevice.brightness_entity,t),150),this._debouncedSetSpread=dt(t=>this._setValue(this._activeDevice.spread_entity,t),150),this._debouncedColor=dt(t=>{this._callService("light","turn_on",{entity_id:this._activeDevice.color_entity,rgb_color:t})},150),this._ov={}}set hass(t){this._hass=t,this._reconcileOverrides(t),this.requestUpdate()}_reconcileOverrides(t){const e=this._activeDevice;if(!e)return;const i={brightness:e.brightness_entity,spread:e.spread_entity};for(const[e,s]of Object.entries(i))if(void 0!==this._ov?.[e]&&Number(t.states[s]?.state)===this._ov[e]){const t={...this._ov};delete t[e],this._ov=t}if(this._wheelHs&&this._sentRgb){const i=t.states[e.color_entity]?.attributes?.rgb_color;i&&i[0]===this._sentRgb[0]&&i[1]===this._sentRgb[1]&&i[2]===this._sentRgb[2]&&(this._wheelHs=void 0)}}get _activeDevice(){return this._devices?.[this._activeIndex]}get _isOn(){return"on"===this._hass?.states[this._activeDevice?.light_entity]?.state}_renderPicker(){return!this._devices||this._devices.length<2?"":q`
      ...
      </ha-card>`}_renderSVG(t,e){const i=t?.attributes?.positionData??[],s=parseFloat(t?.attributes?.sideLength)||150,n=i.filter(t=>8===t.shapeType);if(!n.length)return q`<div style="height:280px"></div>`;const r=function(t){const e={};if(!t)return e;for(const i of t.split(",")){const t=i.indexOf(":");-1!==t&&(e[i.slice(0,t).trim()]=i.slice(t+1).trim())}return e}(e?.state??""),o=s/1.732,a=.75,l=s/1.5,c=s/6,h=n.map(t=>t.x),d=n.map(t=>t.y),p=0-Math.max(...h)-l,u=0-Math.min(...h)+l,_=Math.min(...d)-c,g=Math.max(...d)+c,f=n.map(t=>{const e=function(t,e){const i=parseInt(t.slice(0,2),16),s=parseInt(t.slice(2,4),16),n=parseInt(t.slice(4,6),16),r=Math.max(i,s,n);if(!e||0===r)return"rgb(77,77,77)";const o=255/r;return`rgb(${Math.round(i*o)},${Math.round(s*o)},${Math.round(n*o)})`}(r[String(t.panelId)]??"000000",this._isOn),i=["0,"+-o*a,`${-s/2*a},${o/2*a}`,`${s/2*a},${o/2*a}`].join(" ");return V`
        <g
      ...
      </div>`}_pillDown(t,e,i,s,n){this._pillDragging=!0,t.target.setPointerCapture?.(t.pointerId),this._pillApply(t,e,i,s,n)}_pillMove(t,e,i,s,n){this._pillDragging&&this._pillApply(t,e,i,s,n)}_pillUp(t){this._pillDragging=!1,t.target.releasePointerCapture?.(t.pointerId)}_pillApply(t,e,i,s,n){const r=t.currentTarget.getBoundingClientRect(),o=function(t,e,i,s,n,r){const o=i?(t-e)/i:0,a=s+Math.min(1,Math.max(0,o))*(n-s),l=r?Math.round(a/r)*r:a;return Math.min(n,Math.max(s,l))}(t.clientX,r.left,r.width,e,i,s);this._ov={...this._ov,[n]:o},"brightness"===n?this._debouncedSetBrightness(o):this._debouncedSetSpread(o),this.requestUpdate()}_setValue(t,e){this._callService(t.split(".")[0],"set_value",{entity_id:t,value:e})}_callService(t,e,i,s){this._hass.callService(t,e,i,s)}}customElements.define("nanoleaf-reloaded-card",yt);class mt extends lt{static properties={_config:{state:!0}};static styles=r`
  ...
  `;setConfig(t){this._config=t||{}}set hass(t){this._hass=t,this.requestUpdate()}_options(t){return Object.keys(this._hass?.states??{}).filter(e=>e.split(".")[0]===t).sort()}_update(t,e){const i={...this._config,[t]:e};""===e&&delete i[t],this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}render(){return this._hass?Array.isArray(this._config?.devices)?q`<div class="note">
      ...
      <div class="editor">
        ...
        ${vt.map(t=>{const e=this._config?.[t.key]??"";return q`
            <div class="field">
              <label>${t.label}</label>
              <select
                @change=${e=>this._update(t.key,e.target.value)}
              >
      ...
      </div>`:q``}}customElements.define("nanoleaf-reloaded-card-editor",mt),window.customCards=window.customCards||[],window.customCards.push({type:"nanoleaf-reloaded-card",name:"Nanoleaf Reloaded Card",description:"Control panel for Nanoleaf light panels",preview:!0})}();
